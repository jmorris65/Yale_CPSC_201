...
; ********************************************************
; CS 201 HW #2  DUE Wednesday 9/25/19, 11:59 pm
...
(define (depth tree)
  (cond
    ...
    [(list? (car tree)) (if (> (+ 1 (depth (car tree))) (depth (cdr tree))) (+ 1 (depth (car tree))) (depth (cdr tree)))]
    [else (depth (cdr tree))]))
...
(define (count-if pred tree)
  (cond
    [(empty? tree) 0]
    [(list? (car tree)) (+ (count-if pred (car tree)) (count-if pred (cdr tree)))]
...
(define (tree-replace old new tree)
  (cond
    [(empty? tree) '()]
    [(equal? (car tree) old) (append (list new) (tree-replace old new (cdr tree)))]
