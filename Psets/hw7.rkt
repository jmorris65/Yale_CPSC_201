...
;************************************************************
; CS 201a HW #7  DUE Sunday, December 1st at 11:59 pm, 
...
(define exp5 (concat (either '(a) '(the)) 
                     (concat (either '(mouse) '(cat)) 
                             (either '(ran) '(slept)))))
(define exp6 (concat (repeat '(a)) (repeat '(b))))
...
(define (pick lst)
  (car (drop lst (random (length lst)))))

(define (tester proc arg n [outcomes '()])
  (cond
    [(equal? n 0) outcomes]
    [(list? arg) (let ([out (proc (car arg) (cadr arg))])
            (if (boolean? (assoc out outcomes)) (tester proc arg (- n 1) (cons (list out 1) outcomes))
                (tester proc arg (- n 1) (map (lambda (x) (if (equal? (car x) out) (list out (+ (cadr x) 1)) x)) outcomes))))]
    [else (let ([out (proc arg)])
            (if (boolean? (assoc out outcomes)) (tester proc arg (- n 1) (cons (list out 1) outcomes))
                (tester proc arg (- n 1) (map (lambda (x) (if (equal? (car x) out) (list out (+ (cadr x) 1)) x)) outcomes))))]))
...
(define (generate-string-from-reg-exp exp)
  (cond
    ...
    [(repeat? exp) (if (flip 0.5) (append (generate-string-from-reg-exp (repeat-arg1 exp)) (generate-string-from-reg-exp exp)) '())]
    [else exp]))
...
(define (dfa-accepts? mach str [currstate (dfa-start-state mach)])
  (let* ([symbol (car str)]
         [key (list currstate symbol)]
         [ins (foldl (lambda (x y) (if (equal? (entry-key x) key) x y)) '() (dfa-transitions mach))])
    (cond
      [(empty? ins) #f]
      [(empty? (cdr str)) (not (boolean? (member (entry-value ins) (dfa-accepting-states mach))))]
      [else (dfa-accepts? mach (cdr str) (entry-value ins))])))
...
(define (generate-parse-tree-from-cfg grammar [currstate (cfg-start-symbol grammar)])
  (let ([ins (foldr (lambda (x y) (if (equal? (rule-lhs x) currstate) (cons x y) y)) '() (cfg-rules grammar))])
    (cond
      [(empty? ins) (leaf currstate)]
      [else (let ([usedins (pick ins)]) (node currstate (map (lambda (x) (generate-parse-tree-from-cfg grammar x)) (rule-rhs usedins))))])))
...
(define dfa-mcd
  (dfa
   '(a the mouse cat dog it that slept swam chased evaded dreamed believed)
   '(s sn np det n v vp vi vt)
   's
   '(n vi pn)
   (list
    (entry '(s the) 'sn)
    ...
    (either
     (concat
      ...
      (concat
       ...
       (either '(mouse) (either '(cat) '(dog)))))
     (concat
      (concat (either '(dreamed) '(believed))
              (concat '(that)
                      (either
                       (concat
                        (either '(a) '(the))
...
(define my-cfg
  (cfg
   '(the
     small big fast slow young old
     hyena bull squirrel lion fox cougar
     crushed loved ate rested ran sat
     and while)
   '(sub no n adj ant vb vo vs ca cw)
   'sub
   (list
    (rule 'sub '(no vb))
    (rule 'no '(the adj n))
...
; Possible Outputs
; '(the fast lion rested)
; '(the fast bull ran and the hyena rested and crushed the old bull)
; '(the young lion ate the bull)
; '(the young bull sat and crushed the big lion)
