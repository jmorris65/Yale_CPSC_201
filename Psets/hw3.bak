...
; ********************************************************
; CS 201 HW #3  DUE Friday, 10/4/2019 at 11:59 pm
...
; mach is a representation of a Turing machine config is a
; representation of a configuration of the machine n is the maximum
...
; will account for this, allowing rought twice as many steps for your
...
; creates an instruction with with current state 'q1, current symbol
; 0, new state 'q3, new symbol 1, and move direction 'L, and names it
; i1.
...
; program for a given input.  The alternative is non-deterministism,
; as mentioned in class.

; For this assignment, when writing Turing machien programs (problems
...
(define (i-match? state symbol inst)
  (if (and (equal? state (ins-c-state inst))
           (equal? symbol (ins-c-symbol inst))) #t #f))

(define (i-lookup state symbol mach)
  (if (empty? mach) #f
      (if (i-match? state symbol (car mach)) (car mach)
...
(define (halted? mach config)
  (if (list? (i-lookup (conf-state config) (conf-symbol config) mach)) #f
      (not (i-lookup (conf-state config) (conf-symbol config) mach))))
...
(define (normalize config)
  (let ([newl (reverse (foldl (lambda (x y) (if (and (empty? y) (equal? x 'b)) y (cons x y))) '() (conf-ltape config)))]
        [newr (foldr (lambda (x y) (if (and (empty? y) (equal? x 'b)) y (cons x y))) '() (conf-rtape config))])
    (conf (conf-state config) newl (conf-symbol config) newr)))
...
(define (shift-head-left config)
  (let ([new-symbol (if (empty? (conf-ltape config)) 'b (last (conf-ltape config)))]
        [new-ltape (if (empty? (conf-ltape config)) '() (reverse (cdr (reverse (conf-ltape config)))))]
        [new-rtape (cons (conf-symbol config) (conf-rtape config))])
    (normalize (conf (conf-state config) new-ltape new-symbol new-rtape)))
  )

(define (shift-head-right config)
  (let ([new-symbol (if (empty? (conf-rtape config)) 'b (car (conf-rtape config)))]
        [new-ltape (append (conf-ltape config) (list (conf-symbol config)))]
        [new-rtape (if (empty? (conf-rtape config)) '() (cdr (conf-rtape config)))])
    (normalize (conf (conf-state config) new-ltape new-symbol new-rtape)))
  )
...
(define (next-config mach config)
  (if (halted? mach config) config
      (let* ([instruction (i-lookup (conf-state config) (conf-symbol config) mach)]
            [unshift-config (change-state (ins-n-state instruction) (write-symbol (ins-n-symbol instruction) config))])
        (if (equal? (ins-dir instruction) 'L) (shift-head-left unshift-config) (shift-head-right unshift-config))))
      )
...
(define conf64 (conf 'q1 '() 1 '(0 0 0 0 0 0)))
